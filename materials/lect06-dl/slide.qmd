...
# 1. Define the Model
model = nn.Sequential(
    nn.Linear(4, 512),  # FC Layer
    nn.ReLU(),                           # Activation
    nn.Linear(512, 3) # FC Layer
)
```
...
## Deep Learning with PyTorch: DataLoader
[data file](penguins_size.csv)
...
# 1. Custom Dataset for Penguin Data
class PenguinDataset(Dataset):
    def __init__(self, csv_file):
        df = pd.read_csv(csv_file)
        df = df.dropna()
        # Features: bill_length_mm, bill_depth_mm, flipper_length_mm, body_mass_g
        self.x = torch.tensor(df.iloc[:, 2:6].to_numpy().astype(np.float32), dtype=torch.float32)
        le = LabelEncoder()
        encoded_y = le.fit_transform(df['species'])
        self.y = torch.tensor(encoded_y.astype(np.long), dtype=torch.long)
...
# 2. Split & DataLoader: Batching & Shuffling
from torch.utils.data import random_split

dataset = PenguinDataset('penguins_size.csv')
...
## Deep Learning with PyTorch: Training & Evaluation
